"""
Centralized application configuration for forex_mcp.
...
class Settings(BaseSettings):
    ...
    SCRAPER_TIMEOUT_MS: int = 5000  # 5 seconds (Playwright expects milliseconds)

    # === MCP namespace ===
    NAMESPACE: str = "ffcal"

    # === MCP server transport ===
    MCP_TRANSPORT: str = "http"  # stdio | http | sse
    MCP_HOST: str = "0.0.0.0"  # bind to all interfaces for Docker
    MCP_PORT: int = 3002

    # === HTTP headers for Playwright ===
    USER_AGENT: str = (
    ...
    def local_tz(self) -> ZoneInfo:
        """Get local timezone as ZoneInfo object."""
        try:
